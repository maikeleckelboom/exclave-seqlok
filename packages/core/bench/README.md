...
```bash
pnpm -F @exclave/seqlok run bench:report
```
...
Re-run `pnpm -F @exclave/seqlok run bench:report` whenever you change benches or touch the hot paths.
...
```bash
pnpm -F @exclave/seqlok exec vitest bench bench/observer-reads.bench.ts
```
