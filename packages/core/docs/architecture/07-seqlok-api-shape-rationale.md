...
- Keeps **consumer-side** logic simple:
  ...
  - It just validates the plan/backing pair and produces a `AcceptedHandoff<S>`.
...
- Partitioned allocation strategies (`allocatePartitioned`) normalize to the same logical plan.
...
It might be tempting to treat `plan` as an internal detail of `allocatePacked`:
...
In addition to the golden path `allocatePacked(plan)` (one SAB, contiguous planes), v0.2.0 introduces
...
- `allocatePacked(plan)` / `allocatePartitioned(plan)` → “give me a backing for this plan (contiguous or per-plane SABs).”
...
- `planLayout(spec)`
- `allocatePacked(plan)` / `allocatePartitioned(plan)`
...
export interface SharedWire<S extends SpecInput> {
  ...
  backing: ReturnType<typeof allocatePacked<S>>; // or allocatePartitioned<S>
...
function createSharedWire<S extends SpecInput>(
  ...
  const plan = planLayout(spec);
  const backing = allocatePacked(plan); // or allocatePartitioned(plan) in advanced setups
...
### 6.2 Why ergonomics don't belong in `planLayout` or `allocatePacked`
...
2. `defineSpec → planLayout → allocatePacked → buildHandoff → acceptHandoff → bind*` ← **chosen**
...
- `allocatePartitioned` is the first-class variant for per-plane SABs when that layout is operationally nicer,
...
- `acceptHandoff` implies:
  ...
  - production of a `AcceptedHandoff<S>` typed artefact.
...
flowchart LR
  ...
  B -->|"allocatePacked / allocatePartitioned"| C[backing]
  B --- D[plan hash/meta]
  C --- E["shared memory (SABs / planes)"]
  B --> F[handoff]
  C --> F
  F -->|postMessage| G[accepted]
...
- **backing domain** – `allocatePacked` / `allocatePartitioned` / (advanced) `allocateWasm`
- **handoff domain** – `buildHandoff` / `acceptHandoff` / `verifyHandoff`
