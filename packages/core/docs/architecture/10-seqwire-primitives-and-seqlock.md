...
Primitives live as a **small internal layer** in `@exclave/seqwire`:
...
- Throws `SeqWireError<'internal.assertionFailed'>` if indices are invalid.
...
- On invalid budgets → throws `SeqWireError<'primitives.invalidSpinBudget'>`.
...
- Throws `SeqWireError<'primitives.seqlockTimeout'>` with a `ReadStatus` payload whose `kind` is
  `'budgetExhausted'`.
...
These wrappers normalize errors into structured `SeqWireError`s:

- `SeqWireError<'primitives.atomicsFailed'>`
...
- `SeqWireError<'primitives.invalidSpinBudget'>`
