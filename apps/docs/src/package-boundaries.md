...
```ts
import { defineSpec, planLayout } from "@exclave/seqlok";
import { snapshotCounters } from "@exclave/seqlok/diagnostics";
```

Do not import internal files from `packages/core/src`. Internal modules may change without semver guarantees unless they are exported from `@exclave/seqlok` or `@exclave/seqlok/diagnostics`.
...
Those layers can use `@exclave/seqlok`, but they should not be hidden inside it.
