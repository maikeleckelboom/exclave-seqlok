...
features:
  ...
  - title: Diagnostics and errors
    details: Use environment probes, counters, view descriptions, and structured SeqlokError codes for integration work.
  - title: Package boundary
    details: Import from @exclave/seqlok and @exclave/seqlok/diagnostics; internal modules stay private.
---
...
`@exclave/seqlok` lets a host side define a runtime state contract once, plan its shared-memory layout, hand it across a worker, worklet, or WASM-oriented boundary, and read or write coherent state through role-specific bindings.
...
Start with [Install](/install), follow the [Quickstart](/quickstart), then read the [Seqlok Flow](/core-flow) and [Authored AST vs Runtime](/authoring-contract) pages before integrating across a real worker or worklet boundary.
